...
17. Confirm GitHub Actions secrets are configured: `NPM_TOKEN` and `DOCKERHUB_TOKEN` for Docker Hub user `tobilg`.
18. Run a local release dry run with `make act-release` when release workflow behavior changed.
19. Confirm release artifacts include OS binary archives, `openapi.json`, SHA256 checksum files, npm packages, and the `tobilg/latlng:<tag>` Docker Hub image.
